...
import numpy as np
import io
from solverecaptchas import KOR_MATCH
...
class SolveImage():
    ...
    def __init__(self, page, image_frame, net, tf_net,proxy=None, proxy_auth=None, **kwargs):
        self.page = page
    ...
    async def get_start_data(self):
        ...
        # # Pillow 모듈의 Image.open 메소드로 이미지 열기
            ...
            if self.pieces == 16:
                ...
                if choices:
                    if await self.is_one_selected():
                        await self.cycle_selected(choices) # 3x3만 해당
                        await self.click_verify()
                        if not await self.is_next() and not await self.is_finish():
                            await self.click_reload_button()
                    else:
        ...
        while True:
            ...
            for image_url in images:
                if images != self.download:
                    ...
                    result = await predict(self.net,self.tf_net, file_path,self.title)
    ...
    async def choose(self, image_path):
        ...
        if self.pieces == 9:
            ...
            for i in range(self.pieces):
                # result = await predict(self.net,)
                result = await predict(self.net, self.tf_net,os.path.join(self.cur_image_path, f'{self.image_hash}_{i}.jpg'),self.title)
            ...
            selected = yolov8_image_utils.select_16x16(results_yolov8,numpy_image,self.title)
    ...
    async def get_title(self):
        ...
        if utils.is_korean(title):
            title = KOR_MATCH.convert_to_english(title)
