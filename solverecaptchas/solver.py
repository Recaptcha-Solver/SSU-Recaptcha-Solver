...
class Solver(object):
    ...
    def __init__(self,
        ...
        timeout=30*1000,
        model=Model('model'),
        # net=cv2.dnn.readNet("C:/Users/vvpas/PycharmProjects/recaptcha_solver/model/yolov3.weights", "C:/Users/vvpas/PycharmProjects/recaptcha_solver/model/yolov3.cfg"),
        net=cv2.dnn.readNet(setting.yolov3_weights, setting.yolov3_cfg),
        solve_by_image=True,
    ...
    async def start_test(self,img_path,image_class):
        from solverecaptchas.predict import predict
        im = image.SolveImage(page=None,image_frame=None,net=self.net,tf_net=self.tf_net)
        result = await predict(self.net,self.tf_net,img_path,image_class)
        print("테스트 결과")
        print(result)
    ...
    async def solve_image(self):
        im = image.SolveImage(self.page, self.image_frame, net=self.net,tf_net=self.tf_net)
